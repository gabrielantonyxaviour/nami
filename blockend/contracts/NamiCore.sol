...
contract NamiCore {
    ...
    uint256 public fundingRequestsCount;
    ...
    event AppliedForFunding(
        uint256 fundingRequestId,
        uint256 disasterId,
        address applyingOrgAddress,
        uint256 amount
    );
    event FundClaimInitiated(
        uint256 fundingId,
        address beneficiary,
        Claim[] claims
    );
    ...
    modifier onlyNamiAiClient(bytes32 _sender, uint32 _origin) {
        if (
            _origin != BASE_DOMAIN_ID ||
            bytes32ToAddress(_sender) != namiAiClient
        ) revert InvalidCrosschainCaller(_origin, _sender);
        _;
    }

    modifier onlyBeneficiary(uint256 _fundingId) {
        if (funding[_fundingId].beneficiary != msg.sender)
            revert InvalidCaller(msg.sender);
        _;
    }

    modifier onlyAllowed(uint32 _origin, bytes32 _sender) {
        if (crosschainAddresses[_origin] != _sender)
            revert InvalidCrosschainCaller(_origin, _sender);
        _;
    }

    function initialize(
        address _namiAiClient,
    ...
    function setCrosschainAddresses(
        uint32[] memory _origin,
        bytes32[] memory _addresses
    ) external onlyOwner {
        for (uint256 i = 0; i < _origin.length; i++)
            crosschainAddresses[_origin[i]] = _addresses[i];
    }

    function _createDisaster(
        uint64 _attestationId,
        uint256 _estimatedRequirementInUSD
    ) public {
        ...
        disaster.vaultAddress = IVaultFactory(vaultFactory).createVault(
            disasterCount
        );
        emit DisasterCreated(
            disasterCount,
            _attestationId,
            _estimatedRequirementInUSD,
            disaster.vaultAddress
        );
        disasterCount++;
    }

    function requestFunding(uint256 _disasterId, uint256 _amount) external {
        FundingRequest memory _fundingRequest = FundingRequest({
            disasterId: _disasterId,
            applyingOrgAddress: msg.sender,
            amount: _amount
        });
        fundingRequests[fundingRequestsCount] = _fundingRequest;
        emit AppliedForFunding(
    ...
    function claimFunds(
        uint256 _fundingId
    ) external onlyBeneficiary(_fundingId) {
        ...
        for (uint256 i = 0; i < _claims.length; i++) {
            ...
            if (_claim.chainId != KINTO_DOMAIN_ID) {
                ...
                mailbox.dispatch{value: 0}(
                    _claim.chainId,
                    crosschainAddresses[_claim.chainId],
                    encodedMessage
                );
            } else
                IVaultFactory(vaultFactory).claimFunds(
                    _funding.disasterId,
                    msg.sender,
                    _claim.ethAmount,
                    _claim.wethAmount,
    ...
    ) external payable onlyMailbox onlyNamiAiClient(_sender, _origin) {
        (uint8 _action, bytes memory _params) = abi.decode(
            _data,
            (uint8, bytes)
        );
        if (_action == 0) {
            (uint64 _attestationId, uint256 _estimatedRequirementInUSD) = abi
                .decode(_params, (uint64, uint256));
            ...
            (
                ...
                uint256 _totalAmountInUSD
                    ...
                    _params,
                    (uint64, uint256, address, Claim[], uint256)
                );
            _unlockFunds(
                _disasterId,
                _attestationId,
    ...
    function getDisaster(
        uint256 _disasterId
    ) public view returns (Disaster memory) {
